...
Binary is an example of a number system: it's just a way of writing numbers down. There are many number-writing systems, each providing its own way of writing down every number. Different systems have different ways of representing the same number: for example, if I asked you to write down the number of days in a week, you'd probably write $7$, but if I asked an ancient Roman to do the same thing, the Roman (using Roman numerals) would probably write that as $VII$. Two writing systems, but it's the same number either way!

> If you're now trying to visualize what $7$ means without using a numbering systems, try not to for now. It's an interesting mathematical or philosophical question, but it's not important for this discussion, and personally I think you're likely going to end up more confused than when you started.
>
...
>
> The answer is yup, they probably would. That's why, when we talk about bases, we always use *our* numbering system. So when we say our numbering system is "base 10," we always mean the 10 you learned in grade school.
...
To represent hexadecimal, we have a problem: we the real humans have only invented 10 unique digits, but we need 6 more to represent hexadecimal numbers. By convention, the way you do this is usually to start grabbing letters from the alphabet to stand in for the missing digits. So counting in hexadecimal (or "hex," as it's sometimes called) looks like this:
...
Got it? The rest of the sequence is: 13, 14, 15, 16, 17, 18, 19, 1A, 1B, 1C, 1D, 1E, 1F, 20. If you got that, then perfect!
...
So that's all there really is to binary and hexadecimal; if you get it, you're ready to move on.
...
There are good, non-arbitrary reasons to use these numbering systems; we'll get to those later in the chapter. For now, the short answer is that computer circuitry is based on switching, and binary works nicely with switching: you can represent "switched off" to mean $0$ and "switched on" to mean $1$, and then use sequences of interdependent switches to implement binary calculations by switching this electrical circuitry on or off. We'll see later in the chapter exactly how this works.

Hexadecimal is also widely used because its digits line up evenly with binary digits: every hexadecimal digit corresponds to a four-digit binary sequence, and every four-digit binary sequence corresponds to one hexadecimal digit. This makes it easier (both for people and code) to switch between binary (which the underlying circuitry uses) and hexadecimal, which is a little more compact and easier to read.

> That hex digits line up nicely with binary digits comes from the fact that hex is base-16, and 16 is a power of two. Octal (base-8 numbering) also has this property, but each octal digit corresopnds to a three-digit binary sequence instread of four. 8 and 16 are the powers of two closes to 10, our preferred basis, so you'll see both octal and hex used; of the two, hex is a lot more common.

This brings us to our final topic related to binary and numbering: dealing with large binary numbers, as well as having many binary numbers.

The term **bit** refers to a single digit of a binary number (in fact, the term "bit" is short for "binary digit.") The term **byte** refers to an 8-digit binary number &mdash; a byte is 8 bits. Bytes exist because a single bit has only two possible values, $0$ and $1$, which means you can't do a whole lot with a single bit; a single 8-bit byte can represent any number from 0 to 255, which is a little more useful.

Bytes also explain why hexadecimal is more popular than octal: one hex digit corresopnds to 4 bits, and a byte is 8 bits; so any byte can be represented by a two-digit hexadecimal number.
...
* Hexadecimal is base-16 numbering system useful as a notation for representing bytes
* We use the SI prefixes (kilo, mega, giga, tera, ...) with increments of 1,024 to denote large collections of bytes

With all the numbers and notation stuff out of the way, we're ready to start building electrical circuitry that represents binary numbers. Onward!